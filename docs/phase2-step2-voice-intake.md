...
- **第三方服务**：STT 与 LLM **全部走 DashScope（百炼）**，只依赖一个 `DASHSCOPE_API_KEY`。STT 用 `paraformer-realtime-v2`（非流式调用模式），LLM 用 `qwen-plus` JSON mode。**不**再使用阿里云智能语音交互（NLS）的「录音文件识别极速版」，因此不需要 `ALIYUN_STT_APP_KEY`。参考：[docs/API_docs/实时语音识别.md](API_docs/实时语音识别.md)。
...
### 0.5.1 STT：DashScope Paraformer (`paraformer-realtime-v2`)

- **为什么用"实时"版本做一次性文件识别**：DashScope 上的另一个选项 `paraformer-v2` 是**异步录音文件识别**，提交后返回 `task_id`、轮询到最终结果通常要 10-30s，对我们的 UX（"松手 → 1-2s 后跳到预填页"）完全不可接受。`paraformer-realtime-v2` 虽然叫"实时"，但 SDK 支持**一次性传整段音频、同步返回**的文件调用模式（`callback=None`），30s 音频总耗时 1-2s。
- **为什么不用 `qwen-audio-asr`（录音文件识别-千问）**：它把 STT 和语义理解揉在一个大模型里，单价贵一个数量级（≈ ¥0.06/30s），且与我们"STT 只转文字、qwen-plus 独立做结构化抽取"的分层架构重复付费。保留 STT / LLM 分层也让金标集回归可以分别测试。
- **调用方式**：使用 `dashscope` Python SDK 的 `Recognition(model='paraformer-realtime-v2', callback=None).call(filepath)`，同步调用。**不走 WebSocket**，也**不需要**阿里云 NLS 控制台的 AppKey。参考示例：[docs/API_docs/实时语音识别.md](API_docs/实时语音识别.md) "Paraformer → 识别本地音频文件 → Python"（1703-1733 行）。
- **支持格式**：m4a / aac / mp3 / wav / opus / pcm 等；与 Flutter `record` 包默认输出的 m4a 直接兼容。
- **参数**：`format='m4a'`、`sample_rate=16000`、`language_hints=['zh', 'en']`、`disfluency_removal_enabled=True`（过滤"呃""嗯"类语气词，让 qwen-plus 的 prompt 更干净）。
- **限制**：本步骤前端硬限 ≤ 30s + ≤ 2MB；DashScope Paraformer 单次非流式调用官方建议 ≤ 60s，留足余量。
- **地域**：使用**北京**地域的 DashScope API Key（endpoint `dashscope.aliyuncs.com`）；新加坡地域（`dashscope-intl.aliyuncs.com`）**不提供** `paraformer-realtime-v2`，要留意运维别申错。
- **单次成本**：DashScope Paraformer 按音频时长计价 ≈ ¥0.00008/秒 ≈ **¥0.0024 / 30s**，相较之前的"阿里云 NLS 录音文件识别极速版"（¥0.01/30s）便宜约 **4 倍**。100 活跃用户 × 每天 3 条 ≈ ¥22/月。
...
|---|---|
| MinIO 存储（24h 即删） | ≈ 0 |
| DashScope `paraformer-realtime-v2` | ≈ ¥0.0024 |
...
上传音频 multipart  ──►  POST /api/v1/voice/intake
                            │
                            ├─ STT (DashScope paraformer-realtime-v2, 同步) → transcript
...
3. **STT**：`services/stt.py`（新建，封装 DashScope `paraformer-realtime-v2` 同步调用）；异常 → `stt_failed`，不重试。
...
- 所有第三方 key 从 `config.py` 读 `.env`（只有 `DASHSCOPE_API_KEY` 一个；不再有 `ALIYUN_STT_*` 或 `TONGYI_API_KEY`），符合全局规则。
- 单元测试通过 monkeypatch 注入 fake client 覆盖，不打真实网络。

#### 4.2.1 `services/stt.py` 参考实现

`dashscope` SDK（要求 `dashscope>=1.20`，加到 [backend/requirements.txt](../backend/requirements.txt)）。**关键：`callback=None` = 同步调用，`.call(filepath)` 直接返回完整结果**，适合我们"一次性上传整段 30s 音频"的场景。
...
# SDK 会默认读 DASHSCOPE_API_KEY 环境变量；容器化部署时 .env 已自动注入
# 显式再写一遍方便测试环境注入
dashscope.api_key = settings.DASHSCOPE_API_KEY


class SttUnavailableError(RuntimeError):
    """DashScope STT 暂不可用（上游 5xx / 网络 / SDK 异常）。"""


# MIME → SDK 的 format 参数
_FORMAT_MAP = {
    "audio/m4a": "m4a",
    "audio/aac": "aac",
    "audio/mpeg": "mp3",
...
def transcribe(audio_path: str, mime: str) -> str:
    """同步识别本地音频文件，返回拼接后的文本（含标点）。

    失败时抛 SttUnavailableError；识别到空字符串也当作失败（让调用方走 stt_failed）。
    """
    fmt = _FORMAT_MAP.get(mime)
    if fmt is None:
        raise SttUnavailableError(f"unsupported mime {mime!r}")

    recognition = Recognition(
        model=settings.DASHSCOPE_STT_MODEL,          # 默认 'paraformer-realtime-v2'
        format=fmt,
        sample_rate=16000,
        language_hints=['zh', 'en'],
        disfluency_removal_enabled=True,              # 过滤语气词
        callback=None,                                # ← 关键：同步模式
    )
    ...
    if result.status_code != HTTPStatus.OK:
        logger.warning("dashscope stt non-200: %s %s", result.status_code, result.message)
        raise SttUnavailableError(f"HTTP {result.status_code}: {result.message}")

    # SDK 的 get_sentence() 返回 list[dict]，每个句子有 .text 字段；
    # 文档里也有极简返回 str 的形式（取决于 SDK 版本），做兼容处理。
...
> 离线单元测试：monkeypatch `dashscope.audio.asr.Recognition.call` 返回一个带 `status_code / get_sentence` 的 stub 对象；不动真实网络。
...
|---|---|---|---|---|
| `DASHSCOPE_API_KEY` | 是 | — | ✅ 已配置 | [bailian.console.aliyun.com](https://bailian.console.aliyun.com/?tab=model#/api-key) → API Key 管理 → 创建 sk-… 。**必须选北京地域**（`paraformer-realtime-v2` 新加坡地域不提供）。同一 key 被 [phase2-step3](phase2-step3-photo-auto-assign.md)（多模态 embedding）复用。 |
| `DASHSCOPE_BASE_URL` | 否 | `https://dashscope.aliyuncs.com/compatible-mode/v1` | 默认 | OpenAI 兼容端点，仅被 LLM / embedding 使用；STT 走 dashscope SDK 自带的 endpoint。 |
| `DASHSCOPE_STT_MODEL` | 否 | `paraformer-realtime-v2` | 默认 | 非流式调用（`callback=None`）。升级到 `paraformer-realtime-v3` 时只改此键。 |
...
- 首次配置步骤备忘（运维侧）：
  1. 百炼控制台（北京地域）开通：「通义千问」「Paraformer 实时语音识别」「多模态向量 multimodal-embedding-v1」三项服务；
  2. 在 MinIO 新建 `voice-intake` bucket，挂 24h 生命周期规则；
  3. 把 `DASHSCOPE_API_KEY` 写入生产 `.env`；其它键用默认值即可；
  4. 后端 `requirements.txt` 确保含 `dashscope>=1.20` 与 `openai>=1.40`，随 `docker compose build` 自动安装（**开发者本地不需要 `pip install`**）。
