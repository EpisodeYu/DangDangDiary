...
- **选型历史**：
  ...
  - 2026-04-23 重新评估：[`backend/scripts/llm_bench.py`](../backend/scripts/llm_bench.py) N=3 × 6 case：
    - `qwen-flash`：**p50 1.07s / p90 1.86s / avg 1.48s**，字段命中率 100%（见下）。
    - `qwen-plus`：p50 2.30s / p90 2.56s / avg 2.34s，字段命中率 100%。
    - `qwen3.6-plus`：p50 3.31s / avg 3.35s，慢 40% 且准确率无增益。

    `qwen-flash` 的 `3_days_ago:3` bug 已消失——新快照改成直接输出绝对日期 `2026-04-20`，`_parse_date` 的 `YYYY-MM-DD` 分支本来就能正确归一化。因此定型 `qwen-flash` 为主，砍掉 ~1.2s 的 LLM 端到端延迟。
