...
- **第三方服务**：STT 与 LLM **全部走 DashScope（百炼）**。STT 默认用**新加坡地域**的 `fun-asr`（需要 `DASHSCOPE_API_KEY_SAG`），未配置时自动回落到北京地域的 `paraformer-v1`（`DASHSCOPE_API_KEY`，已有）。LLM 走北京地域 `qwen-plus` JSON mode。**不**再使用阿里云智能语音交互（NLS）的「录音文件识别极速版」，因此不需要 `ALIYUN_STT_APP_KEY`。参考：[docs/API_docs/录音文件识别.md](API_docs/录音文件识别.md)。
...
### 0.5.1 STT：DashScope 新加坡 `fun-asr`（主）+ 北京 `paraformer-v1`（回落）

- **选型历史（两次迭代）**：
  - 2026-04-19 首版：`paraformer-realtime-v2` 同步文件模式（`Recognition(callback=None).call`）。上线后 30s 音频端到端 7-60s+，频繁 "Idle timeout!"：实时模型按实时速度消费音频，且与在线会话共享队列。
  - 2026-04-20 修复：切到 `Transcription.async_call` + **北京 `paraformer-v1`** 异步录音文件识别。30s 音频端到端稳定在 3-5s。
  - 2026-04-21 再次优化：基准测试（`backend/scripts/stt_bench.py`，3.1s 样本 × N=10，服务器在东京）显示跨境握手成为瓶颈——东京→`dashscope.aliyuncs.com` TLS 握手单步 **3.2s**，p50 端到端 6.3s；切到**新加坡**地域（`dashscope-intl.aliyuncs.com`）后握手 **0.08s**，p50 **2.6s**，p90 4.3s，无 ConnectTimeout。Beijing `fun-asr` 在相同样本下出现 134s 长尾，不可选。因此定型：**新加坡 `fun-asr` 作为主路径**，**北京 `paraformer-v1` 作为回落**（SG key 未配置或连续失败时自动切）。
- **为什么不用 `qwen-audio-asr`**：把 STT 和语义理解揉在一个大模型里，单价贵一个数量级（≈ ¥0.06/30s），且与"STT 转文字 / `qwen-plus` 独立做结构化抽取"的分层架构重复付费；分层也让金标集可分别回归。
- **为什么不切到阿里云 NLS「录音文件识别极速版」（FlashRecognizer）**：基准测试里 `audio_address` URL 模式北京/深圳端点 min 可以做到 0.87s，但 10 次里出 1 次 `ConnectTimeout`，p90 9-10s、max 12s，尾延迟比新加坡百炼还差；二进制上传更是 3/5 超时。同时 FlashRecognizer 是另一条产品线（智能语音交互 ISI，非百炼），需要独立维护 NLS 项目、24h Token 轮换、AppKey 管控、独立计费项，运维成本不值。
- **调用方式**：`dashscope.audio.asr.Transcription.async_call(model=..., file_urls=[url], language_hints=['zh', 'en'])` 提交任务 → `Transcription.wait(task=task_id, timeout=15)` 阻塞拿结果 → `httpx.get(transcription_url)` 下载句子 JSON 合并 `text`。音频以 **MinIO 预签名 URL** 的形式传给 DashScope（见 §7 `storage.voice_audio_presigned_url`），不再把字节流经后端二次转发。实现见 `backend/app/services/stt.py`。
- **支持格式**：m4a / aac / mp3 / wav / opus / pcm 等；与 Flutter `record` 默认的 m4a 直接兼容。
- **限制**：前端硬限 ≤ 30s + ≤ 2MB；DashScope 录音文件识别单文件限 12h / 2GB，远超本步骤需求。
- **地域**：主路径用**新加坡** key（`DASHSCOPE_API_KEY_SAG`，endpoint `dashscope-intl.aliyuncs.com/api/v1`，模型 `fun-asr`）；回落用**北京** key（`DASHSCOPE_API_KEY`，endpoint `dashscope.aliyuncs.com/api/v1`，模型 `paraformer-v1`——`fun-asr` 在北京地域长尾严重，不用）。LLM + 多模态 embedding 继续用北京 key，不共用。
- **基准数据（Tokyo 服务器 → 3.1s 16kHz 样本 × N=10，2026-04-21）**：

  | 方案 | 成功率 | p50 | p90 | max |
  |---|---|---|---|---|
  | **SG `fun-asr`（主）** | 10/10 | **2.60s** | **4.33s** | 8.29s |
  | BJ `paraformer-v1`（回落） | 10/10 | 6.34s | 7.24s | 7.81s |
  | BJ `fun-asr`（弃） | 10/10 | 6.28s | 84.2s | 134.67s |
  | NLS FlashRecognizer 深圳 URL | 9/10 | 1.91s | 5.08s | 6.49s + 1 ConnectTimeout |

- **单次成本**：按音频时长计价 ≈ ¥0.00008/秒 ≈ **¥0.0024 / 30s**，回落路径同价。100 活跃用户 × 每天 3 条 ≈ ¥22/月。
...
|---|---|
| MinIO 存储（24h 即删） | ≈ 0 |
| DashScope STT（SG `fun-asr` / BJ `paraformer-v1`） | ≈ ¥0.0024 |
...
上传音频 multipart  ──►  POST /api/v1/voice/intake
                            │
                            ├─ STT (DashScope SG fun-asr 主 / BJ paraformer-v1 回落) → transcript
...
3. **STT**：`services/stt.py`（封装 DashScope `Transcription.async_call`，SG `fun-asr` 主 + BJ `paraformer-v1` 回落）；主路径抛 `SttUnavailableError` 时自动换 region 重试一次；所有 region 都失败 → `stt_failed`。
...
- 薄封装，只暴露 `transcribe(audio_path: str, mime: str) -> str` 和 `extract_intent(transcript: str, context: dict) -> dict`。
- 所有第三方 key 从 `config.py` 读 `.env`（`DASHSCOPE_API_KEY` = 北京，服务 LLM + embedding + STT 回落；`DASHSCOPE_API_KEY_SAG` = 新加坡，服务 STT 主路径。不再有 `ALIYUN_STT_*` 或 `TONGYI_API_KEY`），符合全局规则。
- 单元测试通过 monkeypatch 注入 fake client 覆盖，不打真实网络。

#### 4.2.1 `services/stt.py` 参考实现

源码见 [backend/app/services/stt.py](../backend/app/services/stt.py)（`dashscope>=1.20`，已加到 [backend/requirements.txt](../backend/requirements.txt)）。关键设计：

- 用 **`dashscope.audio.asr.Transcription.async_call(model=..., file_urls=[url], language_hints=['zh','en'])`** 提交任务（不是 `Recognition.call`，也不是 `paraformer-realtime-v2` 的同步文件模式）。`Transcription.wait(task_id, timeout=...)` 阻塞拿结果，再 `httpx.get(transcription_url)` 下载句子 JSON 合并 `text`。
- 两个 region 按优先级串行重试：
  1. **新加坡**：`dashscope.api_key = settings.DASHSCOPE_API_KEY_SAG`，`dashscope.base_http_api_url = 'https://dashscope-intl.aliyuncs.com/api/v1'`，`model = 'fun-asr'`，`wait timeout = 15s`
  2. **北京回落**：`dashscope.api_key = settings.DASHSCOPE_API_KEY`，`base_http_api_url = 'https://dashscope.aliyuncs.com/api/v1'`，`model = 'paraformer-v1'`，`wait timeout = 25s`
- `transcribe(file_url: str) -> str`：`file_url` 是 **MinIO 预签名 URL**（`storage.voice_audio_presigned_url` 返回），DashScope 服务器自己 HTTP 拉取，不过后端。主 region 抛 `SttUnavailableError` 或返回空文本时，自动走下一个 region；全部失败才抛出。
- 基准复测脚本：`backend/scripts/stt_bench.py`（对比 SG `fun-asr` / BJ `paraformer-v1` / BJ `fun-asr` / NLS FlashRecognizer 三地双模式）。

> 离线单元测试：voice_intake 测试链路 monkeypatch `voice_intake_service.stt_transcribe` 这一层，不需要打 dashscope 真实网络。如要测 `stt.py` 内部的 region 回落逻辑，monkeypatch `_submit_and_wait` 按 region 返回不同结果。

#### 4.2.2 `services/llm.py` 参考实现

直接用 `openai` SDK 打 DashScope **OpenAI 兼容端点**（而不是 `dashscope` 原生 SDK）。理由已在 §0.5.2 列出，且与 [phase2-step3](phase2-step3-photo-auto-assign.md) 的 embedding 调用共享同一套 HTTP 客户端约定。
...
_SYSTEM_PROMPT = """..."""  # §5 内容（整段抄过来，一个模块常量）


async def extract_intent(transcript: str, *, default_pet_name: str | None = None) -> dict:
    user_msg = transcript if not default_pet_name else (
        f"（默认宠物名：{default_pet_name}）\n{transcript}"
    )
    try:
        resp = await _client.chat.completions.create(
            model=settings.TONGYI_MODEL,               # qwen-plus
            messages=[
...
|---|---|---|---|---|
| `DASHSCOPE_API_KEY` | 是 | — | ✅ 已配置 | 北京地域 key。[bailian.console.aliyun.com](https://bailian.console.aliyun.com/?tab=model#/api-key)（**不是** modelstudio 子站）→ API Key 管理。服务：`qwen-plus`（LLM）、`multimodal-embedding-v1`（[phase2-step3](phase2-step3-photo-auto-assign.md)）、STT 回落模型 `paraformer-v1`。 |
| `DASHSCOPE_API_KEY_SAG` | 是（推荐） | — | ✅ 已配置 | 新加坡地域 key。[modelstudio.console.aliyun.com](https://modelstudio.console.aliyun.com/?tab=dashboard#/api-key) → API Key 管理。服务：STT 主路径 `fun-asr`。未配置时 STT 会回落到北京 key 的 `paraformer-v1`，p50 从 2.6s 退化到 6.3s（见 §0.5.1 基准）。 |
| `DASHSCOPE_BASE_URL` | 否 | `https://dashscope.aliyuncs.com/compatible-mode/v1` | 默认 | 北京 OpenAI 兼容端点，仅被 LLM / embedding 使用。 |
| `DASHSCOPE_STT_BASE_URL` | 否 | `https://dashscope-intl.aliyuncs.com/api/v1` | 默认 | STT 主路径的 DashScope 原生 endpoint（新加坡）。 |
| `DASHSCOPE_STT_FALLBACK_BASE_URL` | 否 | `https://dashscope.aliyuncs.com/api/v1` | 默认 | STT 回落路径的 endpoint（北京）。 |
| `DASHSCOPE_STT_MODEL` | 否 | `fun-asr` | 默认 | 主路径模型，跑在新加坡地域。只换模型不换 region 时改这里。 |
| `DASHSCOPE_STT_FALLBACK_MODEL` | 否 | `paraformer-v1` | 默认 | 回落路径模型（北京）。不要改成 `fun-asr`——基准实测北京 `fun-asr` 有 >120s 的长尾。 |
...
- 首次配置步骤备忘（运维侧）：
  1. **百炼北京**（[bailian.console.aliyun.com](https://bailian.console.aliyun.com/)）开通：「通义千问」「录音文件识别 Paraformer」「多模态向量 multimodal-embedding-v1」。记 key 到 `.env` 的 `DASHSCOPE_API_KEY`。
  2. **百炼新加坡**（[modelstudio.console.aliyun.com](https://modelstudio.console.aliyun.com/)，是**另一个域名/账号视图**）开通：「Fun-ASR 录音文件识别」。记 key 到 `.env` 的 `DASHSCOPE_API_KEY_SAG`。两个 key 同一个阿里云主账号，计费独立账单合并。
  3. 在 MinIO 新建 `voice-intake` bucket，挂 24h 生命周期规则。
  4. 其它 DashScope 相关 env 键保持默认，仅当需要临时切回北京主路径或换模型时才覆盖。
  5. 后端 `requirements.txt` 确保含 `dashscope>=1.20` 与 `openai>=1.40`，随 `docker compose build` 自动安装（**开发者本地不需要 `pip install`**）。
