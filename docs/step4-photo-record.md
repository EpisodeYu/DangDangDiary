...
2. 后端实现照片缩略图生成
3. Flutter 实现「记录」页面 (选档案、选照片、日期选择)
4. Flutter 实现 EXIF 日期提取
5. 照片记录完成后页面重置
  ...
  "photos": [
    {
      ...
      "photo_url": "http://minio:9000/pet-photos/1/a1b2c3d4.jpg",
      "thumbnail_url": "http://minio:9000/pet-thumbnails/1/a1b2c3d4_thumb.jpg",
      "taken_at": "2024-01-15",
...
3. 逐个处理每张图片:
   a. 验证文件类型和大小
   b. 生成 UUID 文件名
   c. 如果是 HEIC 格式，转换为 JPEG
   d. 上传原图到 MinIO `pet-photos` bucket
   e. 使用 Pillow 生成缩略图 (最大 400x400, JPEG)
   f. 上传缩略图到 MinIO `pet-thumbnails` bucket
   g. 创建 photos 数据库记录
4. 返回所有创建的照片记录
...
用途: 时间轴点击查看原图时使用。使用预签名 URL (1小时过期)，避免直接暴露 MinIO 地址。

---

## 3. 后端实现要点

### 3.1 MinIO 存储服务 (`app/services/storage.py`)
...
async def validate_upload_files(files: list[UploadFile]):
    if len(files) > MAX_FILES_PER_UPLOAD:
        raise HTTPException(400, f"每次最多上传 {MAX_FILES_PER_UPLOAD} 张照片")
    if len(files) == 0:
        raise HTTPException(400, "请至少上传一张照片")
    for f in files:
        if f.content_type not in ALLOWED_CONTENT_TYPES:
            raise HTTPException(400, f"不支持的文件格式: {f.content_type}")
        content = await f.read()
        if len(content) > MAX_FILE_SIZE:
            raise HTTPException(400, f"文件 {f.filename} 超过 15MB 限制")
        await f.seek(0)
```
...
- `backend/app/api/v1/photos.py` - 照片路由 (新建)
...
- [ ] 后端正确验证文件类型 (JPEG/PNG/HEIC/WEBP) 和大小 (<= 15MB)
- [ ] 后端自动生成缩略图并存储到 MinIO
